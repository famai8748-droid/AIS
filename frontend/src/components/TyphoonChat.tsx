// ============================================================
// TyphoonChat — Live chat widget (original design)
// ============================================================
...
const INITIAL_MESSAGE: ChatMessage = {
  id: 'init',
  role: 'ai',
  content: 'สวัสดีครับ! ผมคือ **AI ครูผู้ช่วย (Typhoon v2.5)** ยินดีตอบทุกข้อสงสัยเรื่องบทเรียน ช่วยโจทย์ฝึกฝน หรือแนะนำการค้นหาตัวตนครับ พิมพ์ถามเข้ามาได้เลยครับ! 😊'
};
...
export function TyphoonChat() {
  ...
  const sendMessage = async (text: string) => {
    ...
    setIsSending(true);
    try {
      const data = await api.chatTyphoon(text);
    ...
    <section className="glass-panel" style={{ marginTop: '32px', padding: '24px 32px', borderColor: 'rgba(0,230,118,0.25)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', paddingBottom: '12px', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: '#fff' }}>สนทนาสดกับ AI ครูผู้ช่วย (Typhoon v2.5 Live Chat)</h3>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontSize: '12px', color: '#00E676', fontFamily: 'monospace' }}>Typhoon Cloud API</span>
        </div>
      </div>
          ...
          <div key={msg.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
            ...
            {msg.isLoading ? (
              <div className="chat-bubble-ai" style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#00E676' }}>
                ...
                ⚡ Typhoon v2.5 กำลังคิดและพิมพ์ตอบ...
              </div>
            ) : msg.role === 'ai' ? (
      ...
      {/* Input Bar */}
      <form id="form-typhoon-chat" onSubmit={e => { e.preventDefault(); sendMessage(input); }} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        <input
          id="chat-input"
          type="text"
          value={input}
          onChange={e => setInput(e.target.value)}
          placeholder="พิมพ์ข้อความคุยกับ Typhoon AI..."
          disabled={isSending}
          style={{
