...
# Async OpenAI client for Typhoon AI
async_client = AsyncOpenAI(
    api_key="sk-5dGrv8Dl6WTeMHWkctsZlJZkBZVxqi6oBJRWcGVNn5y61IfF",
...
class TyphoonService:
    ...
    def _load_dataset(self):
        ...
        if os.path.exists(DATASET_PATH):
            try:
                ...
                print(f"[TyphoonService] Loaded {len(dataset)} items from career_dataset_th.jsonl")
            except Exception as e:
                print(f"[TyphoonService] Error loading dataset: {e}")
        return dataset

    async def is_typhoon_online(self):
        try:
            # OpenTyphoon's /models returns a list instead of an object with a 'data' field,
            # which crashes the official openai client's list model parser. We use httpx directly.
            import httpx
            async with httpx.AsyncClient() as client:
                res = await client.get(f"{async_client.base_url}models", headers={"Authorization": f"Bearer {async_client.api_key}"})
    ...
    async def chat_with_typhoon(self, user_message):
        ...
        if await self.is_typhoon_online():
            try:
                ...
                if reply:
                    return {
                        "status": "success",
                        "source": "Typhoon v2.5 30B (OpenTyphoon API) — async",
                        "reply": reply
                    }
            except Exception as e:
                print(f"[TyphoonService] Async chat error: {e}")
...
"""
        if await self.is_typhoon_online():
            try:
                ...
                if text:
                    return {"status": "success", "source": "Typhoon v2.5 30B (OpenTyphoon API) — async", "content": text}
            except Exception as e:
                print(f"[TyphoonService] Async career guidance error: {e}")
...
"""
        if await self.is_typhoon_online():
            try:
                ...
                print(f"[TyphoonService] Async remedial error: {e}")
